...
<h3 style="margin: 0;padding: 0;font-family: 'HelveticaNeue-Light', 'Helvetica Neue Light', 'Helvetica Neue', Helvetica, Arial, 'Lucida Grande', sans-serif;line-height: 1.1;margin-bottom: 15px;color: #000;font-weight: 500;font-size: 27px;">
    Wimbo wenye jina {{ $song->name }} haujawawekwa kwenye tovuti.
</h3>
    <p class="lead" style="margin: 0;padding: 0;font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif;margin-bottom: 10px;font-weight: normal;font-size: 17px;line-height: 1.6;">
        ...
        Baada ya wimbo {{$song->name}} kupitiwa na kufanyiwa uhakiki na kamati ya uhakiki. Umeonekana unahitaji kufanyiwa marekebisho. Tafadhali fanyia kazi mapendekezo yafuatayo:<br><br>
        
        @foreach($failedReviews as $failedReview)
            <p>
                {{$loop->iteration}}. {!! $failedReview->question !!}: <strong>HAPANA</strong><br>
                @if(isset($comments[$failedReview->review_question_id]))
                    <br><strong>Mapendekezo swali la {{$loop->iteration}}.</strong><br>
                    {!! $comments[$failedReview->review_question_id] !!}
                @endif
            </p>
        @endforeach
